...
if [ -n "$LOG_FILES" ]; then
    ...
    for log_file in $LOG_FILES; do
        if [ -f "$log_file" ]; then
            ...
            grep -i "注册的全局过滤器\|全局过滤器\|GlobalFilter\|TokenCheckFilter" "$log_file" | tail -20
        fi
    done
else
...
echo "=== 测试接口验证过滤器是否生效 ==="
echo "测试白名单接口:"
curl -s -o /dev/null -w "HTTP状态码: %{http_code}\n" http://localhost:8081/test/public

echo "测试受保护接口(无token):"
curl -s -o /dev/null -w "HTTP状态码: %{http_code}\n" http://localhost:8081/test/protected 